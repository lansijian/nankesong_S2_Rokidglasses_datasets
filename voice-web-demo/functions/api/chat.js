...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (!text) {
      return json({ ok: false, error: "EMPTY_INPUT", message: "请先说点什么。" }, 400);
    }
    ...
    return json({
      ...
      answer,
      transcript: text,
      snippets: snippets.map((item) => ({ source: item.source, tags: item.tags }))
    });
  } catch (error) {
    return json(
      {
...
export async function onRequestGet() {
  return json({ ok: true, service: "chat", platform: "cloudflare-pages", message: "POST text to this endpoint." });
}
...
async function generateWithModel({ env, mode, region, text, history, snippets, scene, replyGoal, roastIntensity }) {
  ...
  const context = snippets.map((item, index) => `${index + 1}. [${item.tags.join(",")}] ${item.text}`).join("\n");
  const workflowContext = mode === "roast"
    ? `当前场景：${scene}\n当前怼人强度：${roastIntensity}\n当前目标：给一句能直接说出口的回怼话，并保持不越线。`
    : `当前场景：${scene}\n当前回复目标：${replyGoal}\n请按高情商随身军师的流程，先读懂场面，再给用户一句能落地的话。`;

  const response = await fetchWithRetry(`${(env.OPENAI_BASE_URL || "https://api.deepseek.com/v1").replace(/\/$/, "")}/chat/completions`, {
    method: "POST",
    headers: {
